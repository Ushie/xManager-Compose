...
class API(cronet: CronetKnetEngine, val json: Json) {
    ...
    suspend fun ping(): Int {
        return try { withContext(Dispatchers.Main) {
    ...
    suspend fun download(workdir: File, name: String, url: String): File {
        val out = workdir.resolve(name)
        if (out.exists()) {
            return out
        }
        val file = withContext(Dispatchers.IO) {client.execute(HttpRequest(HttpMethod.GET, url)).body!!.asBytes()}
        out.writeBytes(file)
        return out
    }
